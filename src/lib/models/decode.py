...
def _nms_wh(heat, wh, kernel_list=(1, 3, 5),
            area_list=([0 ** 2, 32 ** 2], [32 ** 2, 96 ** 2], [96 ** 2, 1e5 ** 2])):
    # 计算面积
    area = wh[:, 0, :, :] * wh[:, 1, :, :].unsqueeze(1)
    res = torch.zeros(heat.size()).cuda()
    for i, k in enumerate(kernel_list):
        pad = (k - 1 ) // 2
        hmax = nn.functional.max_pool2d(
            heat, (k, k), stride=1, padding=pad)
        keep = (hmax == heat).float()
        area_mask = ((area >= area_list[i][0]) & (area < area_list[i][1])).\
            float().expand(res.size()).cuda()
        res = res + area_mask * keep
...
def rodet_decode(heat, wh, angle, reg=None, cat_spec_wh=False, cat_spec_angle=False, K=100):
    batch, cat, height, width = heat.size()

    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    heat = _nms(heat)
    # heat = _nms_wh(heat, wh)
    scores, inds, clses, ys, xs = _topk(heat, K=K)
    if reg is not None:
        reg = _tranpose_and_gather_feat(reg, inds)
        reg = reg.view(batch, K, 2)
    ...
    if cat_spec_angle:
        ...
        wh = wh.gather(1, clses_ind).view(batch, K, 1)
    else:
    ...
    #                     xs + wh[..., 0:1] / 2,
    #                     ys + wh[..., 1:2] / 2], dim=2)
    rbboxes = torch.cat([xs, ys, wh[..., 0:1], wh[..., 1:2], angle] , dim=2)
    detections = torch.cat([rbboxes, scores, clses], dim=2)
