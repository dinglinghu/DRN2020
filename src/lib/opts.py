...
class opts(object):
	def __init__(self):
		...
		self.parser.add_argument('--fp16', action='store_true',
								 help='turn on the training with fp16.')
		self.parser.add_argument('--e2e', action='store_true',
								 help='turn on the training in end-to-end manner.')
		# log
		...
		self.parser.add_argument('--lr_step', type=str, default='20,90,120',
		...
		self.parser.add_argument('--keep_res', action='store_true',
														 help='keep the original resolution'
																	' during validation.')
		self.parser.add_argument('--gene_guided_grad_cam', action='store_true',
								 help='generate guided grad cam for heatmap')
		...
		self.parser.add_argument('--hm_meta_weight', type=float, default=0.1,
								 help='loss weight for dynamic classification.')
		self.parser.add_argument('--test_meta_cls', action='store_true', help='switch for meta_cls test')
		self.parser.add_argument('--test_meta_reg', action='store_true', help='switch for meta_reg test')
		self.parser.add_argument('--finetune', action='store_true', help='drop head params if finetune')
		self.parser.add_argument('--fsm', action='store_true', help='switch for feature selection module')
		self.parser.add_argument('--rot', action='store_true', help='switch for feature selection module with rcn')
		self.parser.add_argument('--drmc', action='store_true', help='switch for Dynamic Refine module for classification')
		self.parser.add_argument('--drmr', action='store_true', help='switch for Dynamic Refine module for regression')
		self.parser.add_argument('--only_ls', action='store_true', help='switch for only retain the loss of last stack')
		self.parser.add_argument('--all_cls_nms', action='store_true', help='switch for nms across all cls')
		self.parser.add_argument('--test_split', type=int, default=-1, help='select the test split in testing phase')
		self.parser.add_argument('--number_stacks', type=int, default=2, help='number stacks for hourglass network.')
		# multi_pose
		...
		# rodet
		self.parser.add_argument('--rotate_binloss', action='store_true',
														 help='apply rotate bin loss not mse loss')
		self.parser.add_argument('--dense_angle', action='store_true',
														 help='apply weighted regression near center or '
																	'just apply regression on center point.')
		self.parser.add_argument('--cat_spec_angle', action='store_true',
														 help='category specific bounding box angle.')
		self.parser.add_argument('--cache_model', type=int, default=1000000,
								 help='the number of epochs to save models')
		# exdet
		...
		self.parser.add_argument('--center_thresh', type=float, default=0.03,
		...
		self.parser.add_argument('--eval_oracle_angle', action='store_true',
														 help='use ground truth bounding box angle.')
	...
	def init(self, args=''):
        # 注意这里只是默认值，虽然设置了，但是后面sku.py里面数据会更新
		default_dataset_info = {
			'ctdet': {'default_resolution': [768, 768], 'num_classes': 1,
			...
			'ddd': {'default_resolution': [384, 1280], 'num_classes': 3, 
								...
								'dataset': 'kitti'},
			'rodet': {'default_resolution': [768, 768], 'num_classes': 1,
					'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
					'dataset': 'rosku'},
		}
		class Struct:
