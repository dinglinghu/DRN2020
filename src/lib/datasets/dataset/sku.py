...
class SKU(data.Dataset):
  ...
  def __init__(self, opt, split):
    ...
    self.img_dir = os.path.join(self.data_dir,'images', '{}sku'.format(split))
    if split == 'test':
      self.annot_path = os.path.join(
          self.data_dir, 'cocoformatJson', 
          'voc_sku110k_aug_test.json').format(split)
    else:
      if opt.task == 'exdet':
        self.annot_path = os.path.join(
        ...
        self.annot_path = os.path.join(
          self.data_dir, 'cocoformatJson', 
          'voc_sku110k_aug_{}.json').format(split)
    self.max_objs = opt.K
    self.class_name = ['__background__', 'drink']
    ...
    print('==> initializing sku-110K {} data.'.format(split))
