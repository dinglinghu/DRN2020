...
class HRSC(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        self.data_dir = os.path.join(opt.data_dir, 'HRSC2016')
        # self.img_dir = os.path.join(self.data_dir, 'images', '{}'.format(split))
        self.img_dir = os.path.join(self.data_dir, 'FullDataSet', 'AllImages')
        if split == 'test':
            self.annot_path = os.path.join(
                self.data_dir, 'cocoformatJson',
                'voc_HRSC2016_test444.json').format(split)
        else:
            if opt.task == 'exdet':
                self.annot_path = os.path.join(
                ...
                self.annot_path = os.path.join(
                    self.data_dir, 'cocoformatJson',
                    'voc_HRSC2016_{}.json').format(split)
                split = 'train'
        self.max_objs = opt.K
        self.class_name = ['__background__', 'ship']
        # 所有的id list，新数据注意更改.
        ...
        print('==> initializing HRSC2016 {} data.'.format(split))
