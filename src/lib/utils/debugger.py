...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    att_colors = [(att_color_list[_]).astype(np.uint8) \
              for _ in range(len(att_color_list))]
    cnv_colors = [(cnv_color_list[_]).astype(np.uint8) \
              for _ in range(len(cnv_color_list))]
    self.att_colors = np.array(att_colors, dtype=np.uint8).reshape(len(att_colors), 1, 1, 3)
    self.cnv_colors = np.array(cnv_colors, dtype=np.uint8).reshape(len(cnv_colors), 1, 1, 3)
    if self.theme == 'white':
  ...
  def add_blend_img(self, back, fore, img_id='blend', trans=0.7, white=False):
    if white:
      fore = 255 - fore
  ...
  def gen_colormap(self, img, output_res=None, att=False, angle=False, cnv=False):
    img = img.copy()
    ...
    if att:
        cp = np.max(img, axis=2).reshape(h,w,1,1)
        img = img-cp
        img[cp.reshape(h,w)<0.1,:,:] = -1
        img = (img >=  0).astype(np.float32)

        colors = np.array(
            self.att_colors, dtype=np.float32).reshape(-1, 3)[:c].reshape(c, 3)
    elif cnv:
        colors = np.array(
            self.cnv_colors, dtype=np.float32).reshape(-1, 3)[:c].reshape(c, 3)
    else:
        colors = np.array(
          self.colors, dtype=np.float32).reshape(-1, 3)[:c].reshape(c, 3)
    # if self.theme == 'white':
    #   colors = 255 - colors
    if att:
        color_map = colors[np.argmax(img,axis=2).reshape(-1)].reshape(h,w,3).astype(np.uint8)
        color_map[cp.reshape(h,w)<0.1,:] = np.array([255,255,255])
    else:
  ...
  def add_rbbox(self, rbbox, cat, conf=1, show_txt=True, img_id='default', gt=False):
    # NOTE bbox角度变成整数就不准确了
    bbox = np.array(rbbox, dtype=np.int32)
    # print(rbbox[0], rbbox[1], rbbox[2], rbbox[3], rbbox[4])
    ...
    font = cv2.FONT_HERSHEY_SIMPLEX
    cat_size = cv2.getTextSize(txt, font, 0.5, 2)[0]
    # 不是bbox[4],因为bbox[4]是整数，不准确
    # ####### rosku的json文件角度是-0.5pi到0.5pi ##########
    if self.dataset == 'rosku':
        angle = (rbbox[4] - 0.5) * np.pi
    # ####### dota的json文件角度是0到2pi ##########
    else:
        angle = rbbox[4]
    pt1, pt2, pt3, pt4 = self._get_four_points((bbox[0], bbox[1]), angle, bbox[2], bbox[3])
    ...
    # cv2.circle(img, (int(cx), int(cy)), 5, (0, 0, 255), 4)
    # print(pt1, pt2, pt3, pt4)
    pt1 = self._float_to_int(pt1)
    pt2 = self._float_to_int(pt2)
    pt3 = self._float_to_int(pt3)
    pt4 = self._float_to_int(pt4)

    # pt1, pt2, pt3, pt4 = bound_correct(list(pt1), list(pt2), list(pt3), list(pt4),W,H)
    if gt:
        cv2.line(self.imgs[img_id], pt1, pt2, [128, 0, 128], line_width)
    else:
        cv2.line(self.imgs[img_id], pt1, pt2, [0, 0, 128], line_width)
    cv2.line(self.imgs[img_id], pt2, pt3, color, line_width)
    cv2.line(self.imgs[img_id], pt3, pt4, color, line_width)
    cv2.line(self.imgs[img_id], pt4, pt1, color, line_width)
  ...
  def save_img(self, imgId='default', path='./cache/debug/'):
    ...
    cv2.imwrite(path + '/{}.png'.format(imgId+time_str), self.imgs[imgId])
...
sku_class_name = ['drink']

dota_class_name = [ 'plane', 'baseball-diamond', 'bridge', 'ground-track-field',
                'small-vehicle', 'large-vehicle', 'ship',
                'tennis-court', 'basketball-court',
                'storage-tank', 'soccer-ball-field',
                'roundabout', 'harbor',
                'swimming-pool', 'helicopter']
hrsc_class_name = ['ship']
...
att_color_list = att_color_list.reshape((-1, 3)) * 255

cnv_color_list = np.array(
        [
            1.000, 1.000, 1.000,
            0.000, 1.000, 0.000,
        ]
    ).astype(np.float32)
cnv_color_list = cnv_color_list.reshape((-1, 3)) * 255
